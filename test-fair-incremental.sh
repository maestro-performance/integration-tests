...
function testIbmMqLight() {
    ...
    docker run -it -h maestro_client -v maestro:/maestro --network=maestro_cluster -e PRODUCT_NAME="IBM MQ Light" maestro-test-client /usr/bin/test-runner-fair-incremental.sh
    sleep 30s
...
function testArtemis() {
    ...
    docker run -it -h maestro_client -v maestro:/maestro --network=maestro_cluster -e PRODUCT_NAME="Artemis 2.6.3" maestro-test-client /usr/bin/test-runner-fair-incremental.sh
    sleep 30s
...
function testActiveMQ() {
    ...
    docker run -it -h maestro_client -v maestro:/maestro --network=maestro_cluster -e PRODUCT_NAME="ActiveMQ 5.15.2" maestro-test-client /usr/bin/test-runner-fair-incremental.sh
    sleep 30s
...
function testQpidDispatch() {
    ...
    docker run -it -h maestro_client -v maestro:/maestro --network=maestro_cluster -e PRODUCT_NAME="Interconnect 1.4.0" maestro-test-client /usr/bin/test-runner-fair-incremental.sh
    sleep 90s
...
function testQpidCpp() {
    ...
    docker run -it -h maestro_client -v maestro:/maestro --network=maestro_cluster -e PRODUCT_NAME="Qpid CPP" maestro-test-client /usr/bin/test-runner-fair-incremental.sh
    sleep 90s
    docker-compose -f docker-compose.yml -f docker-interconnect-compose.yml down
    docker rmi -f maestro_sut || true
}
...
function testRabbitMq() {
    ...
    10s
    docker run -it -h maestro_client -v maestro:/maestro --network=maestro_cluster -e PRODUCT_NAME="RabbitMQ" -e SEND_RECEIVE_URL="amqp://sut:5672/test.performance.queue?protocol=RABBITAMQP" maestro-test-client /usr/bin/test-runner-fair-incremental.sh
    sleep 30s
